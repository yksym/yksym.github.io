...
* オレンジ色のプログラム意味論を読む(2回目)
    ...
    * その代わり「圏論の歩き方」という本で単純型付ラムダ計算（λ → )のCCCに基づく表示的意味論のさらっとした紹介読んで満足した
...
-- パターン2: f(n+1) = h(f(n),n) <- primitive recursion: 原始再帰的関数
fact :: Nat -> Nat
...
```
パターン1: f(x:xs) = h(x, f(xs))       <- h は 注目している要素と残り全ての要素の処理結果(f xs) にのみ依存している点に注意。
パターン2: f(x:xs) = h(x, f(xs), xs)   <- パターン1と異なり、未処理の情報も使える(最初に条件に合致したものは処理して後はそのまま流用とか出来る）
パターン3: ????                        <- 勉強中。さっぱり分からん
```
...
```
    * 再帰関数のメモ化: https://wiki.haskell.org/Memoization
...
F  : 関手
X  : 型
~= : 同型を表す演算子。ここでは型はただの集合として、要素間の1:1対応付が可能であることを意味する（つまり逆写像がある)
